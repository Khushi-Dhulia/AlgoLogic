...
export default function CircularQueue() {
  ...
  const enqueue = async () => {
    ...
    if (isEmpty()) {
      ...
      if (newRear === 0) setMessage("🔁 Rear wrapped to index 0");
      else setMessage(`✅ "${value}" added at index ${newRear}`);
    }
  ...
  const dequeue = async () => {
    ...
    if (front === rear) {
      ...
      if (newFront === 0) setMessage("🔁 Front wrapped to index 0");
      else setMessage(`Removed "${removed}"`);
    }
      ...
      <div className="flex gap-8">
        ...
        <div className="w-64 border rounded-xl p-6 bg-white shadow space-y-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div>Front Index: {front !== -1 ? front : "None"}</div>
            <div>Rear Index: {rear !== -1 ? rear : "None"}</div>
            <div>Is Full: {isFull() ? "Yes" : "No"}</div>
          </div>
